...
class AudioTranscriber:
            ...
            while self.transcribing:
                try:
                    ...
                    for segment in segments:
                        try:
                            await self.gpt_interface.transcribed_text.put(segment.text)
    ...
    def process_audio(self, audio_data: np.ndarray):
        ...
        if (
            not is_speech and self.silence_counter > self.app_options.silence_limit
        ) or len(self.audio_data_list) > 5:
            self.silence_counter = 0
    ...
    def batch_transcribe_audio(self, audio_data: np.ndarray):
        ...
        if self.gpt_interface is not None:
    ...
    def text_proofreading(self, segment_list: list):
        ...
        result = self.gpt_interface.text_proofreading(combined_text)
    ...
    async def start_transcription(self):
        try:
            ...
            logger.warning(
                "Exception occurred while starting transcribe {}".format(str(e))
            )
